...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0.3, adaptive=True, hid_dim=64):   
        ...
        self.gate_inter_time = FrequencyAwareGatedConv2d(base_channel, base_channel*8, kernel_size=(3, 3), stride=(2, 1), padding=(1, 1))
        self.gate_inter_freq = FrequencyAwareGatedConv2d(base_channel, base_channel*8, kernel_size=(3, 3), stride=(2, 1), padding=(1, 1))
    ...
    def forward(self, x, x_freq, label=None,feat=False):
        ...
        feat_time_final = self.cls_time(torch.cat((freq_feat, kxa1), dim=3)) 

        kxa2 = self.gate_inter_freq(freq_shallow, cross=True)
        feat_freq_final = self.cls_freq(torch.cat((time_feat, kxa2), dim=3))  



        if label!=None:
            rep_joint = self._generate_mask(x.clone(), bs, M, label)#NM 1 T V 
